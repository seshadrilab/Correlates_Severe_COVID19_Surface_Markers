...
xml_path_b1 <- here::here("data/20200527 HAARVI DURT B1/20200527 HAARVI B1 CS.xml")
...
(plot(gs_b1, fontsize=15))
dev.off()

save_gs(gs_b1, here::here("out/GatingSets/GatingSet_B1"))

#####################################################################

# gs_b1 <- load_gs(here::here("out/GatingSets/GatingSet_B1"))
...
gates2draw <- list("/Time", "/Time/S", "/Time/S/L", "/Time/S/L/3+",
                   c("/Time/S/L/3+/4+", "/Time/S/L/3+/8+"), 
                   c("/Time/S/L/3+/CD38+", "/Time/S/L/3+/HLADR+"), 
                   "/Time/S/L/3+/GD+",
                   "/Time/S/L/3+/GD+/Vd2+", 
                   c("/Time/S/L/14+", "/Time/S/L/19+"),
                   "/Time/S/L/56+",
                   "/Time/S/L/iNKT", 
                   "/Time/S/L/MAIT")

# list.append(gates2draw, rCD1b_mock_path, rCD1b_GMM_path)
...
cd4_mem_gates <- c("/Time/S/L/3+/4+/CCR7+", "/Time/S/L/3+/4+/CD45RA+")
cd8_mem_gates <- c("/Time/S/L/3+/8+/CCR7+", "/Time/S/L/3+/8+/CD45RA+")

for(currentGates in list(cd4_mem_gates, cd8_mem_gates)) {
  png(filename = file.path(here::here("out/QC/FACS_Plots"),
                           sprintf("%s_B1_%s.png",
                                   sub(".*\\/([^\\/]+$)", "\\1", currentGates[[1]]),
                                   format(Sys.time(), "%Y-%m-%d_%H-%M-%S"))),
      width = 1900, height = 1030)
  print(plotter_mem(gs_b1, currentGates))
  dev.off()
}
