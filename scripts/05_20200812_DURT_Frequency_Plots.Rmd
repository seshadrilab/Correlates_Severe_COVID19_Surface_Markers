...
plot_result <- function(col) {
  ...
  plot <- plot + 
    annotate("text", x = 1.5, y = plot_ylims[1] + 0.9*diff(plot_ylims), label = p.adj.text, size=7)
  if(!(col %in% c("Monocytes_Lymphocytes_ratio", "CD4_CD8_ratio"))) {
    plot <- plot + scale_y_continuous(labels = function(x) paste0(x*100))
  }
  plot
}
...
freqs_vs_cohort_plots <- lapply(gl, function(x) {
  x[['widths']] = wd
  x})
...
if(save_output) {
  for(col in cols_to_test) {
    ...
    print(freqs_vs_cohort_plots[[col]])
    dev.off()
}
}
```
...
for(col in cols_to_test) {
  grid.arrange(freqs_vs_cohort_plots[[col]])
}
```
...
wd <- do.call(unit.pmax, lapply(gl, "[[", 'widths')) # Grab the width of the plot which has the largest width
freqs_vs_days_plots <- lapply(gl, function(x) {
  x[['widths']] = wd
  x})
for(freq in freqs_to_plot_vs_days) {
  grid.arrange(freqs_vs_days_plots[[freq]])
}
```
